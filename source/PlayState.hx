...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'gaming':
				dialogue = CoolUtil.coolTextFile(Paths.txt('gaming/dialogue'));
			case 'anger':
				dialogue = CoolUtil.coolTextFile(Paths.txt('anger/dialogue'));
			case 'engage':
				dialogue = CoolUtil.coolTextFile(Paths.txt('engage/dialogue'));
		}
		...
		switch(stageCheck)
		{
			...
			case 'bg':
				{
					defaultCamZoom = 0.9;
					curStage = 'bg';
					var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('not-bob/bg'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
		 			bg.active = false;
		    		add(bg);
					...
					bgPeople.frames = Paths.getSparrowAtlas('not-bob/Characters');
					bgPeople.animation.addByPrefix('bop', 'Characters', 24, false);
					bgPeople.y += 600;
					bgPeople.x += 400;
			...
			case 'bg-fire':
				{
					defaultCamZoom = 0.9;
					curStage = 'bg-fire';
					var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('not-bob/bg-fire'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
		 			bg.active = false;
		    		add(bg);
			...
			case 'bg-big-fire':
				{
					defaultCamZoom = 0.9;
					curStage = 'bg-big-fire';
					var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('not-bob/bg-big-fire'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
		 			bg.active = false;
		    		add(bg);
				}
			case 'bg-intense-fire':
				{
					defaultCamZoom = 0.9;
					curStage = 'bg-intense-fire';
					var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('not-bob/bg-intense-fire'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
		 			bg.active = false;
		    		add(bg);
        ...
        if (SONG.player2 == 'jeremy-mad')
		{
			jeremyT = new FlxTrail(dad, null, 3, 24, 0.2, 0.039);
			add(jeremyT);
		}
	...
	*/
	...
	override function stepHit()
	{
		...
		if (SONG.song.toLowerCase() == 'pyrophobia')
		{
			switch(curStep)
			{
				case 2360:
					remove(jeremyT);
					changeDadCharacter('jeremy-guitar');
					dad.playAnim('grab');
		        case 2873:
					dad.playAnim('out');
				case 2880:
					changeDadCharacter('jeremy-mad');
			}
		}
